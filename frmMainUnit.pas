...
const
  OgwAnalytics = '<!--#include virtual="/ogw/includes/analytics.inc.html"-->';
  InsertText = '    ' + OgwAnalytics;
  EndHead = '</head>';
var
  ALine: string;
  CharIndex: Integer;
  ...
  JQueryPos: Integer;
begin
  OgwSearch := LowerCase(OgwAnalytics);
  ...
  try
    ...
    begin
      ...
      if Pos(OgwSearch, LowerCaseLine) > 0 then
      begin
        Break;
      end;
      ...
      begin
        ...
        if BreakLine then
        begin
          ALine := Copy(ALine, 1, EndHeadPos - 1) + sLineBreak + OgwAnalytics
